...
if __name__ == "__main__":
    ...
    img_to_show = "00611" # Specify here which image you want to check the groundtruth for

    print(f"Converting and showing bboxes for image {img_to_show}")

    for (rgb, bboxes, cls, dth), fpath, _ in d:
        if fpath[-8:-3] == img_to_show:
            ...
            for bbox, cl in zip(bboxes, cls):
                ...
                axs[1].add_patch(r)
                plt.annotate(cl.item(), xy=(x0, y0), xytext=(x0+0.2, y0+0.2))
            text_cls = []
            for label in cls:
                text_cls.append(d.label_names[label])
            fig.suptitle(cls)
            print(text_cls)
            plt.show()
    print("Done")