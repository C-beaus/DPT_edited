...
class SelmaDrones(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return self.to_pytorch(rgb, vboxes, dth), fpath, (h,t)
...
if __name__ == "__main__":
    d = SelmaDrones("c:/Users/chase/OneDrive/Documents/Grad/ML_for_Robots/final_project/Town01_Opt_120_color",
                    "c:/Users/chase/OneDrive/Documents/Grad/ML_for_Robots/final_project/splits")

    image_width = 1920
    image_height = 1080
    if d.split_type == "test":
        d.split_type = "val"
    output_dir = os.path.join("c:/Users/chase/OneDrive/Documents/Grad/ML_for_Robots/final_project/dataset/labels", d.split_type)
    os.makedirs(output_dir, exist_ok=True)
    # train_dir = os.path.join(output_dir, "train")
    # val_dir = os.path.join(output_dir, "val")
    # os.makedirs(train_dir, exist_ok=True)
    # os.makedirs(val_dir, exist_ok=True)
    print("Converting and saving bboxes")

    for (rgb, bboxes, cls, dth), fpath, _ in d:
        # fig, axs = plt.subplots(1,2)
        # axs[0].imshow(d.to_rgb(rgb.permute(1,2,0)))
        # axs[1].imshow(d.to_rgb(rgb.permute(1,2,0)))
        if bboxes.numel() == 0:
            # Save to corresponding file
            output_file = os.path.join(output_dir, fpath[-8:-3] + ".txt")
            print(f"Image {fpath[-8:-3]} contains no valid bounding boxes")
        ...
        for bbox, cl in zip(bboxes, cls):
            ...
            yolo_annotation = f"{cl} {x_center} {y_center} {width} {height}\n"
